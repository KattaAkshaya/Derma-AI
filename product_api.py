...
def fetch_products_from_dataset(skin_type, skin_issues=None, user_concerns=None, min_rating=4):
    ...
    # Collect relevant product types based on skin issues and concerns
    ...
    # If no specific concerns, use general skin-type recommendations
    if not relevant_products and skin_type in skin_type_to_products:
        relevant_products.update(skin_type_to_products[skin_type])

    # Return empty DataFrame if no relevant product types found
    if not relevant_products:
        return pd.DataFrame()

    # Filter dataset based on relevant product types and minimum rating
    filtered_products = df[(df["ProductType"].isin(relevant_products)) & (df["Rating"] >= min_rating)]
    
    # Return empty DataFrame if no matching products are found
    if filtered_products.empty:
        return pd.DataFrame()

    # Remove duplicate ProductIds (keeping highest-rated one per type)
    filtered_products = filtered_products.drop_duplicates(subset=["ProductId"])

    # Ensure diverse selection: Group by ProductType & pick highest-rated product in each type
    diverse_products = (filtered_products
